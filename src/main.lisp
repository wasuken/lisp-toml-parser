...
(defun deep-namespace (table namespaces)
  (unless (gethash (intern (car namespaces) :keyword) table)
	(setf (gethash (intern (car namespaces) :keyword) table)
		  (make-hash-table)))
  (if (cdr namespaces)
	  (deep-namespace (gethash (intern (car namespaces) :keyword) table)
					  (cdr namespaces))
	  (gethash (intern (car namespaces) :keyword) table)))
		  ...
		  (t
		   (let* ((namespace (ppcre:regex-replace-all "\\[|\\]" (car lst) ""))
			 ...
			 (cond ((= (length namespaces) 1)
					(unless (gethash (intern namespace :keyword) *toml*)
					  (setf (gethash (intern namespace :keyword) *toml*) (make-hash-table)))
					(setf (gethash (intern (if (integerp (car (car (cdr lst))))
											   (write-to-string (car (car (cdr lst))))
											   (car (car (cdr lst))))
										   :keyword)
								   (gethash (intern namespace :keyword) *toml*))
						  (nth 2 (car (cdr lst)))))
				   (t
  ...
  (let ((text ""))
	(with-open-file (in path)
	  (loop for line = (read-line in nil)
		 while line
		 do (setf text (format nil "~a~%~a" text line))))
	(parse text)))
