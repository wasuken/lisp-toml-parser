(defpackage toml-parser/tests/main
  (:use :cl
        :toml-parser
        :rove))
(in-package :toml-parser/tests/main)
...
(defun table-test (table expected)
  (let ((kv-lst '()))
	(maphash #'(lambda (k v)
			 ...
			 table)
	(ok (equal (reverse kv-lst)
			   expected))))

(defun table-plist-test (table expected)
  (ok (equal (toml-parser:deep-hashtable-to-plist table)
			 expected)))
...
test2=1") '((:|test| . "te===st2") (:|test2| . 1)))
  ...
  (testing "table test"
	...
	(ok (equal (toml-parser:toml-key-list)
			 '(:|test| :|hoge|)))
...
(deftest errors
  (testing "error mechanism"
	(ok (signals (toml-parser:parse "test ========= \"aa\"")))
	(ok (signals (toml-parser:parse "i3qu0@34-0;wafioadkopew:k:pjpvnfprwfpewiefio")))
	(ok (signals (toml-parser:parse "¥12]4]5[4¥322:12]:21:43:456:5765__89]890]0--jk]jy]re[3e]f")))
	(ok (signals (toml-parser:parse "/mcxzjiof23489sadk:")))))
